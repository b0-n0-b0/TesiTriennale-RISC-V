...
void vga_init(char *vga_framebuffer) {
  ...
  init_textmode_80x25();
  print_VGA("Hello RISC-V world!", 0x00, 0x32);
  // init_graphicmode_320x300();
  // for (int x = 0; x < 320; x++) {
  //   for (int y = 0; y < 300; y++) {
  //     vga_framebuffer[y * 320 + x] = 0x00 ;
  //   }
  // }
  //
  // // prints a square as a PoC of printing on the VGA Graphic Mode Framebuffer
  // for (int i = 0; i < 20; i++) {
  //   for (int j = 0; j < 20; j++) {
  //     vga_framebuffer[(80+j)*320 + 150+i] = 0xff;
  //   }
  // }
}
